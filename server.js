...
app.post('/api/guardar-datos', (req, res) => {
    ...
    // datos.forEach(dato => {
    //     const {ID, Name, Open_Time, Fecha} = dato;
    //     const opentime = new Date(dato.Open_Time).toISOString().slice(0,19).replace('T',' ');
    //     const query = 'INSERT INTO data (ID, Name, Open_Time, Fecha) VALUES (?, ?, ?, ?)';

    //     db.query(query, [dato.ID, dato.Name, opentime, dato.Fecha], (err, result) => {
    //         if (err) {
